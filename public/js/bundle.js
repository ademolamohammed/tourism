...
console.log('hello from the client side'); //const locations = JSON.parse(document.getElementById('map').dataset.locations) this was imported to index.js
...
var displayMap = function displayMap(locations) {
  ...
  var map = new mapboxgl.Map({
    ...
    scrollZoom: false // center: [-118.1135,34.1174],
    // zoom: 10,
    // interactive:false 
  ...
  locations.forEach(function (loc) {
    ...
    new mapboxgl.Popup({
      offset: 30
    }).setLngLat(loc.coordinates).setHTML("<p>Day ".concat(loc.day, ": ").concat(loc.description, " </p>")).addTo(map); //extends mark bounds to inlude current location
...
var showAlert = function showAlert(type, msg) {
  hideAlert();
  var markup = "<div class=\"alert alert--".concat(type, "\"> ").concat(msg, " </div>"); //this ${type} is what is been parsed to it, and its takes 
...
var stripe = Stripe('pk_test_WcYD3aaObukec6r3r3th7NtQ00ZLJj6myT');
...
//use to make newer javascript features work perfectly in older browsers
...
if (mapBox) {
  var locations = JSON.parse(mapBox.dataset.locations); //for the map
...
if (userDataForm) {
  userDataForm.addEventListener('submit', function (el) {
    el.preventDefault();
    var form = new FormData(); //the FormData()is from the multer. must be included before image  or any files can be uploaded
...
if (userPasswordForm) {
  userPasswordForm.addEventListener('submit', function _callee(el) {
    var passwordConfirm, password, confirmPassword;
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            ...
            passwordConfirm = document.getElementById('password-current').value;
...
if (bookBtn) {
  bookBtn.addEventListener('click', function (el) {
    ...
    var tourId = el.target.dataset.tourId; // can also use const tourId=document.getElementById('book-tour').dataset.tourId
